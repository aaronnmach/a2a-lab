...
def main() -> None:
    ...
    with (
        A2AClient(echo_url) as echo_client,
        A2AClient(reverse_url) as reverse_client,
    ):
        echo_client.fetch_agent_card()
        reverse_client.fetch_agent_card()

        original = "Hello from the chained multi agent demo"
        echo_result = echo_client.extract_text(
            echo_client.send_task(original)
        )
        reverse_result = reverse_client.extract_text(
            reverse_client.send_task(echo_result)
        )
